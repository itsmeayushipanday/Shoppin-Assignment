            ...
            <header className='w-full bg-white shadow-md sticky top-0 z-10 flex justify-between items-center px-6 py-4'>
                ...
                <div className='flex items-center'>
                    ...
                    <div className='relative group'>
                        <div
                            className='ml-2 mr-2 p-2 rounded-full hover:bg-gray-200 cursor-pointer'
                            title='Google Apps'
                        >
                    ...
                    <a
                        href='#'
                        className='hover:underline text-sm text-slate-500'
                    >
                        Google Account
                    </a>
                </div>
                ...
                <div className='flex-1 bg-gray-50 p-4 overflow-auto h-screen'>
                    <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 auto-rows-fr'>
                        {data.map((item) => (
                            <div
                                key={item.id}
                                className={`border rounded-lg overflow-hidden border-gray-200 ${getRandomGridSize()}`}
                            >
                                {/* Image */}
                                <Image
                                    src={item.image}
                                ...
                                {/* Title */}
                                <h3 className='text-black font-bold px-2 pt-2'>
                                    {item.smallTitle}
                                </h3>

                                {/* Truncated Description */}
                                <p
                                    className='text-gray-600 px-2 pb-2 truncate'
                                    style={{ maxWidth: '100%' }}
                                >
                                    {item.description}
                                </p>
